...
We expect heterogeneity values to vary substantially between studies, and thus, we will use a *half-Cauchy* prior for the heterogeneity parameter, such that $\tau \sim \mbox{half-Cauchy(0.25)}$ (See figure \@ref(fig:tauprior)). This is a *Cauchy* distribution truncated at zero (constrained into a positive real value), which has a fatter tail than a half-normal distribution, and offers a ***proper weakly informative prior*** for the heterogeneity parameter with 70% of expected values between 0 and 0.5 [@roverWeaklyInformativePrior2021]. As a sensitivity check, we will rerun the analyses with two other parameters: an intermediate prior with scale set to $\tau \sim \mbox{half-Cauchy(0.5)}$, and a less informative prior with scale set to $\tau \sim \mbox{half-Cauchy(1)}$.

```{r tauprior, fig.cap = "Heterogeneity priors", fig.align='center', warning = F, message = F, echo = F, out.width='85%'}
...
prior_tau %>% parse_dist(prior_tau) %>% 
            ...
            scale_fill_brewer(palette = "Dark2",
                              ...
                              labels = c("Prior ~ Half-Cauchy (0.25)",
