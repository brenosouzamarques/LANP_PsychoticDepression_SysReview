...
A Bayesian framework requires that prior beliefs about the data are made explicitly clear. These are presented not as point estimates but as parameters that have probability distributions. In the context of a meta-analysis, we are mostly concerned with three parameters: **$\mu_i$**, the mean of effects derived from point estimates within included studies $i$; **$\tau^2$**, the heterogeneity parameter that quantifies how within-study effects vary from one study to the other; and **$\theta$**, which represents a distribution of the 'true' effect that is **$\sim N(\mu, \sigma)$**. Further information on Bayesian meta-analysis and parameters of interest is available elsewhere [@roverBayesianRandomEffectsMetaAnalysis2020; @roverWeaklyInformativePrior2021; @williamsBayesianMetaAnalysisWeakly2018].

\newpage

### Prior specification - Effect parameter $\mu$ {#maineffect}

We will use a ***weakly informative prior*** that is normally distributed, so that the main effect **$\sim N(\mu, \sigma^2)$** with a location parameter of **0** and a scale of **.5** (See figure \@ref(fig:ggdistpriors)). This specification means that 95% of expected values lie between -1 and 1. As a sensitivity analysis, we look for the largest available meta-analysis in the field [@zaninottoCognitiveMarkersPsychotic2015], which gives a mean difference for individuals with PMDD of -0.30 on a 95% confidence interval of -0.57 to -0.19. This gives a standard error of around $SE = 0.2$ [@greenCochraneCollaborationCochrane2011] which we will use as an ***informative prior*** that is normally distributed **$N \sim \mbox{(-0.3, 0.2)}$**, so that roughly 80% of expected values lie between -0.5 and 0.5. Finally, estimates from a ***uniform*** prior **$\sim (-1, 1)$** will also be used for comparison, which assumes that all possible values between -1 and 1 are equally likely *a priori*. Parameter estimates and model results will be compared analytically and with the aid of Bayes Factors.

```{r ggdistpriors, fig.cap = "Weakly, Informative and Uniform priors", fig.align='center', warning = F, message = F, echo = F, out.width='85%'}
library(extraDistr)
library(tidyverse)
library(brms)
library(ggdist)
...
From the combination of priors for **$\mu$** and **$\tau$**, a total of six meta-models will be conducted, two for each of the three priors of for the effect and the two priors for heterogeneity. The values of **$\theta$** from the combination of the Uniform and Informative priors for the effect with both priors for heterogeneity (four in total), will be considered as exploratory sensitivity analysis for our main model (See \@ref(fig:thetaprior)). The latter consists of the Weakly Informative prior for **$\mu$**, as was previously described in \@ref(maineffect) of this preregistration protocol. We aim to include all models for the sensitivity analysis as supplemental material to the future manuscript.

```{r thetaprior, fig.cap = "$\\theta$ prior distributions for $\\tau \\sim \\mbox{half-Cauchy(0.5)}$ and every prior for $\\mu$", fig.align='center', warning = F, message = F, echo = F, out.width='85%'}
